...
| 功能 | vmlinux-to-elf | kernel_lenz | 状态 |
|------|----------------|-------------|------|
| **Token Table 发现** | ✅ 搜索 "0123456789" + 避免特定模式 | ✅ 完整实现 | ✅ 已实现 |
| **Token Index 验证** | ✅ 自动推导偏移 | ✅ 构建 LE/BE 模式匹配 | ✅ 已实现 |
| **Markers 验证** | ✅ 递增检查 + 范围 0x200-0x4000 | ✅ 相同范围 | ✅ 已实现 |
| **符号数量验证** | ✅ 最小 256 符号 + DP 算法 | ⚠️ 仅范围检查 | 可改进 |
| **地址零值检查** | ✅ 超过 20% 则重试 | ✅ 已实现 | ✅ 已实现 |
| **负偏移启发式** | ✅ 位掩码检测 + 警告 | ✅ 已实现 | ✅ 已实现 |
| **候选质量评分** | ⚠️ 简单选择 | ✅ 质量评分系统 | ✅ 已实现 |
| **OpenWRT 未压缩格式** | ✅ 回退支持 | ❌ 未实现 | 可选改进 |
| **配置覆盖参数** | ✅ `--override-relative-base` | ❌ 未实现 | 可选改进 |
| **符号类型验证** | ✅ 验证类型字符有效性 | ⚠️ ELF 构建时过滤 | 可改进 |
| **基址合理性检查** | ⚠️ 简单高位检查 | ✅ 页对齐可选 + 模式检查 | ✅ 已实现 |
